...
def get_sell_signals(tickers: List[str]) -> List[Dict[str, Any]]:
    ...
    for t in tickers:
        # Skip fundamental and distribution analysis for non‑equity instruments
        # such as futures contracts (e.g. "HG=F").  These tickers typically
        # lack inventory, revenue and receivables data and will generate
        # warnings if passed into fundamental or distribution functions.
        if '=' in t:
            fundamental = None
            distribution = None
        else:
        ...
        # Technical signals can still be computed for most traded instruments
